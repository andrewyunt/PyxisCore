...
public class BankConversation implements ConversationAbandonedListener {
	...
	private int prompt, amount;
	private String action, prefix = ChatColor.DARK_GRAY + "[" + ChatColor.YELLOW + "Pyx" + ChatColor.GOLD + "Bank"
	...
	public BankConversation() {
		...
		conversationFactory = new ConversationFactory(PyxisCore.getInstance()).withModality(true)
				.withFirstPrompt(new CommandPrompt()).withEscapeSequence("quit").withTimeout(30)
		...
		public String getPromptText(ConversationContext context) {
			...
			if (prompt == 1)
				return prefix + ChatColor.GRAY
						+ "What action would you like to perform? (Possible choices: deposit/withdraw)";
			else
	...
	public void conversationAbandoned(ConversationAbandonedEvent event) {
		...
		if (!event.gracefulExit())
			TownyMessaging.sendErrorMsg((CommandSender) who,
					"Command execution stopped by " + event.getCanceller().getClass().getName());
		else
